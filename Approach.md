...
| Image size, channels | 192×192; `top`=RGB, `xray`=RGB (near‑grayscale: max channel diff ≈ 38 to 54), `seed`/`mask`=L | Train at **native 192×192**; keep x‑ray as its own channel(s). |
...
| Foreground fraction of the 192×192 image | **mean ≈ 3.5%, range 0.7 to 9.3%** | Severe class imbalance → region/topology losses (Dice/Tversky/clDice), **not** plain BCE. |
...
```
 ...
 top.png ──▶│ R G B                                                              │
xray.png ──▶│       Xgray (+ optional CLAHE / dark-metal channel)                │ ─▶ 192×192 × (4 to 6) ch
...
- **First‑conv adaptation:** replace the 3‑channel stem with a 4 to 6‑channel conv; copy pretrained RGB weights into the RGB channels and initialize the extra channels (x‑ray, seed) from the mean of the pretrained weights (standard transfer‑learning channel‑inflation).
...
5. **Phase 4, ensemble & polish.** Ensemble folds + best 2 to 3 backbones (probability averaging), optional **Fast‑Marching** refinement, final RLE sanity checks, submission.
