...
|---|---|---|
| **GPU** | 1× **T4 / P100 / RTX 3060 to 4090 / A10 / L4**, **12 to 16 GB VRAM** | Headroom for U‑Net++, larger batches, mixed precision, dual‑encoder fusion, and the **clDice loss overhead** (≈ +88% training time, ≈ +52% VRAM vs Dice‑only, budget for it). |
| **CPU** | 8+ cores | Faster albumentations pipeline (elastic/affine are CPU‑bound). |
| **RAM** | 16 GB | Cache decoded images in memory. |
| **Disk** | 5 to 10 GB | 5 CV folds × multiple backbones × checkpoints + TTA artifacts. |

### 1.3 Training‑time budget (rough, on a single T4‑class GPU)
- One model, 192×192, resnet34 U‑Net, ~100 to 150 epochs on 260 images: **~30 to 90 min**.
- **5‑fold CV** (the recommended setup): **~3 to 6 hours** total.
...
|---|---|---|
| **Python 3.10 to 3.12** | runtime | 3.12 is present locally; any 3.10+ is fine. |
...
| Question | Answer |
|---|---|
| GPU needed? | **Yes, 1 GPU** (≥6 GB min, 12 to 16 GB recommended). Local env is CPU‑only → use web/cloud. |
